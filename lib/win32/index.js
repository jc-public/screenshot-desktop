...
function copyToTemp () {
  const tmpExe = path.join(os.tmpdir(), 'screenCapture', 'screenCapture_1.3.2.exe')
  const tmpManifest = path.join(os.tmpdir(), 'screenCapture', 'app.manifest')
  const includeExe = path.join(__dirname, 'screenCapture_1.3.2.exe').replace('app.asar', 'app.asar.unpacked')
  ...
  const sourceData = {
    exe: fs.readFileSync(includeExe),
    manifest: fs.readFileSync(includeManifest)
  }
  fs.writeFileSync(tmpExe, sourceData.exe)
...
function windowsSnapshot (options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const tmpExe = copyToTemp()

    exec('"' + tmpExe + '" "' + imgPath + '" ' + displayChoice, {
